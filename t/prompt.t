...

{
  ...
  is( _prompt("Yes or no?", "no"), "no",  "prompt returned default when input exhausted" );
};

{
  ...
  is( _prompt("Yes or no?", "no"), "no", "prompt returned default under PERL_MM_USE_DEFAULT" );
};

{
  local *STDIN = _set_tempfile("yes");
  is( _prompt("Yes or no?", "no"), "no", "prompt returned default when not interactive" );
};
